...
void DWIN_Channel_ErrorFromISR(dwin_channel_t *ch) {
  ...
  /* dwin2: если экрана на этом канале физически нет (выключен в настройках,
   * enabled_flag=0) — его RX-линия обычно висит в воздухе и ловит наводки
   * НЕПРЕРЫВНО (реально наблюдалось: ~50 ошибок/сек на отключённом главном
   * экране). needs_resync ниже форсит полный пересинк ВСЕХ каналов сразу
   * (см. force_full_resync в main.c) — без этой проверки такой "шум в
   * никуда" на выключенном канале постоянно долбил принудительным ресинком
   * и другой, реально используемый канал, из-за чего его зеркалируемые поля
   * то и дело перезатирались прямо во время редактирования пользователем.
   * error_count/rx_state/re-arm приёма всё равно обновляем как обычно —
   * это чистая диагностика и не мешает каналу снова заработать, если его
   * потом включат. */
...
void DWIN_PatchFrameAddress(uint8_t *payload, uint8_t payload_len, int32_t delta) {
  ...
  /* dwin2: адреса ниже 0x5000 — это СИСТЕМНЫЕ регистры DWIN (например,
   * 0x0010 — текущее время встроенных часов экрана, см. request_rtc_time()
   * в main.c проекта основного блока), а не пользовательские VP. У них нет
   * "локального"/"проводного" домена — это один и тот же адрес по всей
   * цепочке плат, транслировать (сдвигать) его нельзя. Раньше при пересылке
   * через мост (эта функция вызывается тут ДО DWIN_SendRawFrame) такой кадр
   * молча патчился как обычный пользовательский VP — запрос времени долетал
   * до второго экрана уже с испорченным адресом и не находил там нужного
   * системного регистра. Все реальные пользовательские VP этого проекта
   * лежат в 0x5000+ (см. Library/pool_types.h), поэтому порог безопасен. */
  if (addr < 0x5000) {
    return;
  }
...
void DWIN_Channel_Poll(dwin_channel_t *ch, uint32_t now_tick) {
  if (ch->rx_state != DWIN_RX_WAIT_HEADER1) {
    ...
    if (idle_ms > DWIN_RX_STUCK_TIMEOUT_MS) {
      ...
      ch->rx_state = DWIN_RX_WAIT_HEADER1;
      ch->restart_count++;
      /* dwin2: та же защита, что и в DWIN_Channel_ErrorFromISR — см. её
       * комментарий. Выключенный канал (шумящая в никуда линия) не должен
       * форсить пересинк остальных. */
      if (DWIN_Channel_IsEnabled(ch)) {
