...
/* === usart_error: таймаут "завис посреди кадра" для канала выносного
 * экрана (USART1) === */
// Физически этот канал НЕ прямой провод — основной блок и плата-посредник
// соединены через пару модулей NT1-M (serial<->Ethernet) и интернет.
// Пинг непредсказуем (зависит от провайдера), поэтому короткий таймаут
// прямого провода (200 мс, DWIN_RX_STUCK_TIMEOUT_MS_DEFAULT в dwin.c) тут
// не годится — он будет ложно сбрасывать ещё живой, просто задержавшийся
// в сети кадр. Ложное срабатывание запускает полный цикл ресинка (см.
// DWIN_FULL_RESYNC_PERIOD_MS выше и needs_resync в dwin.h), а это ДОЛЬШЕ,
// чем просто подождать чуть дольше настоящего конца кадра — поэтому здесь
// стоит щедрое значение, не тесное. Скорость реакции на пользователя
// определяет не этот таймаут (он вообще не участвует, пока кадр приходит
// целиком без обрыва), а сама сеть — тут её всё равно не ускорить.
// Используется через DWIN_Channel_SetStuckTimeout(&dwin_remote, ...) в main.c.
#define DWIN_REMOTE_STUCK_TIMEOUT_MS 5000
