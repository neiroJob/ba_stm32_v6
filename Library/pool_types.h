...
/* === Период фонового heartbeat'а публикации расписания на сервер ===
 * Расписание публикуется отдельным JSON-сообщением по USART2 СОБЫТИЙНО
 * (сразу после нажатия "Сохранить" на экране), плюс подстраховкой на
 * случай потери того самого пакета — фоновым повтором раз в этот период.
 * Специально НЕ шлётся в общем статусном JSON каждые 800мс (см.
 * StartMqttWrite) — расписание меняется на порядки реже, раздувать
 * горячий пакет ради него незачем (и рискованно, см. историю бага с
 * обрезанием JSON от переполнения таймаута передачи). 5 минут — разумный
 * компромисс между "сервер быстро узнает об изменении, если событийный
 * пакет потерялся" и "не спамим то, что почти никогда не меняется". */
#define SCHEDULE_HEARTBEAT_PERIOD_MS 300000
