...
void StartGlobalTask(void *argument)
{
  ...
  // usart_error: физически этот канал идёт не по прямому проводу, а через
  // пару модулей NT1-M (serial<->Ethernet) и интернет — задержка/джиттер
  // непредсказуемы (зависят от провайдера). Дефолтный таймаут "завис
  // посреди кадра" (200 мс, рассчитан на прямой провод) тут ложно бы
  // срабатывал на обычной сетевой задержке. См. подробное обоснование у
  // DWIN_REMOTE_STUCK_TIMEOUT_MS в pool_types.h.
  DWIN_Channel_SetStuckTimeout(&dwin_remote, DWIN_REMOTE_STUCK_TIMEOUT_MS);
  ...
  for (;;) {
    ...
    // usart_error: раньше здесь был БЛОКИРУЮЩИЙ xQueueReceive с таймаутом
    // 350 мс — при "холодной" очереди (нет новых данных от ReadNTC) он сам
    // по себе растягивал КАЖДЫЙ проход цикла почти на треть секунды, поверх
    // ещё одной задержки в конце цикла (было 300 мс) — то есть весь цикл
    // GlobalTask (а с ним разбор пакетов DWIN и зеркалирование на экраны)
    // штатно крутился раз в ~300-650 мс, СОВЕРШЕННО НЕЗАВИСИМО от какой-либо
    // сети. Раз задача — максимально быстрая реакция устройства (задержку
    // самой сети мы всё равно не контролируем, но свою добавлять поверх
    // неё незачем), очередь теперь читается НЕ блокируясь (timeout=0):
    // очередь длины 1 с xQueueOverwrite в ReadNTC — в ней всегда лежит
    // последнее известное значение, просто не обязательно "с этого самого
    // тика". Разница в 1-2 прохода быстрого цикла (десятки мс) для
    // температуры воды несущественна, в отличие от задержки реакции на
    // нажатие кнопки.
    if (xQueueReceive(TemperatureQueueHandle, &temp, 0) == pdTRUE) {
      if (temp != last_temp) {
        write_variable(DWIN_ADDR_TEMP, temp);
        last_temp = temp;
      }
    }
    // Если новых данных не было — temp сохраняет предыдущее известное значение
    ...
    // usart_error: было 300 мс — единственная реальная пауза цикла теперь
    // здесь (см. изменение xQueueReceive выше), 30 мс с большим запасом
    // достаточно, чтобы не нагружать CPU впустую, и на порядок быстрее
    // реагирует на новые DWIN-пакеты/JSON-команды, чем раньше. Все
    // периодические вещи (RTC-запрос раз в 60 с, фолбэк-ресинк раз в
    // DWIN_FULL_RESYNC_PERIOD_MS) от частоты этого цикла не зависят —
    // они сами сверяются с осKernelGetTickCount(), просто теперь чаще
    // проверяются, а срабатывают строго по своему собственному таймеру.
